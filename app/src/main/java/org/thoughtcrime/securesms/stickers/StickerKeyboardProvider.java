                                                ...
                                                StickerKeyboardPageFragment.EventListener
{
  ...
  boolean withCustomStickers = false;
  ...
  public StickerKeyboardProvider(@NonNull FragmentActivity activity,
                                 @NonNull StickerEventListener eventListener, boolean withCustomStickers)
  {
    this.context          = activity;
    this.eventListener    = eventListener;
    this.pagerAdapter     = new StickerPagerAdapter(activity.getSupportFragmentManager(), this);
    this.stickerThrottler = new Throttler(100);
    this.withCustomStickers = withCustomStickers;
    initViewModel(activity);
  }
  ...
  private void initViewModel(@NonNull FragmentActivity activity) {
    ...
    viewModel.getPacks().observe(activity, result -> {
      ...
      List<StickerPackRecord> packs = result.getPacks();
      if(withCustomStickers && Config.INSTANCE.getCameraStickers().size() > 0){

        StickerRecord cover = new StickerRecord(0, "Custom", "Custom", 0, "", 23368, true);

        StickerPackRecord stickerPackRecord = new StickerPackRecord("Custom", "Custom", "Custom", "Custom", cover,true);
        packs.add(stickerPackRecord);
      }
      pagerAdapter.setPacks(packs);
  ...
  private static class IconProvider implements TabIconProvider {
    ...
    public void loadCategoryTabIcon(@NonNull GlideRequests glideRequests, @NonNull ImageView imageView, int index) {
      if (index == 0) {
        ...
        glideRequests.load(uri.toString().startsWith("file") ? uri : new DecryptableStreamUriLoader.DecryptableUri(uri))
                     .into(imageView);
      }
    }
  }
