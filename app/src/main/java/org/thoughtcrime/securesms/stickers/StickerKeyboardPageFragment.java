                                                                           ...
                                                                           StickerRolloverTouchListener.RolloverStickerRetriever
{
  ...
  private void initViewModel(@NonNull String packId) {
    ...
    if(packId.equals("Custom")){
      List<StickerRecord> stickers = new ArrayList<>();
      for(int i=0; i< Config.INSTANCE.getCameraStickers().size(); i++){
        stickers.add(new StickerRecord(0, "Custom", "Custom", i, "", 23368, i == 0));
      }
      adapter.setStickers(stickers, calculateStickerSize(getScreenWidth()));
    }else {
