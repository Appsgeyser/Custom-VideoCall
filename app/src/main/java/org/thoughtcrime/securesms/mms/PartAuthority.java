...
//  private static final Uri    STICKER_CONTENT_URI = Uri.parse(STICKER_URI_STRING);
  ...
  private static UriMatcher getUriMatcher(Context context){
    ...
    uriMatcher.addURI(context.getPackageName(), "part/*/#", PART_ROW);
    uriMatcher.addURI(context.getPackageName(), "thumb/*/#", THUMB_ROW);
    uriMatcher.addURI(context.getPackageName(), "sticker/#", STICKER_ROW);
    uriMatcher.addURI(DeprecatedPersistentBlobProvider.getAUTHORITY(context), DeprecatedPersistentBlobProvider.EXPECTED_PATH_OLD, PERSISTENT_ROW);
    uriMatcher.addURI(DeprecatedPersistentBlobProvider.getAUTHORITY(context), DeprecatedPersistentBlobProvider.EXPECTED_PATH_NEW, PERSISTENT_ROW);
    uriMatcher.addURI(BlobProvider.getAUTHORITY(context), BlobProvider.PATH, BLOB_ROW);
    return uriMatcher;
  }
  ...
  public static Uri getAttachmentDataUri(AttachmentId attachmentId) {
    Uri uri = Uri.withAppendedPath(PART_URI_STRING(ApplicationContext.getInstance().getApplicationContext()), String.valueOf(attachmentId.getUniqueId()));
    return ContentUris.withAppendedId(uri, attachmentId.getRowId());
  }

  public static Uri getAttachmentThumbnailUri(AttachmentId attachmentId) {
    Uri uri = Uri.withAppendedPath(THUMB_URI_STRING(ApplicationContext.getInstance().getApplicationContext()), String.valueOf(attachmentId.getUniqueId()));
    return ContentUris.withAppendedId(uri, attachmentId.getRowId());
  }

  public static Uri getStickerUri(long id) {
    return ContentUris.withAppendedId(STICKER_URI_STRING(ApplicationContext.getInstance().getApplicationContext()), id);
  }

  public static boolean isLocalUri(Context context, final @NonNull Uri uri) {
    int match = getUriMatcher(context).match(uri);
    switch (match) {
    case PART_ROW:
    case THUMB_ROW:
